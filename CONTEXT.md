...
**Bound**:
The maximum number of Records an effectful Step may affect, declared by the Step's author, and at least
one — a Bound admitting no Record is refused where it is written, being a guardrail no Step can act
under rather than the strictest there is. Mandatory on a `destroy` Step, where an absent Bound means
unbounded rather than unchecked, and refused on an Opaque one, where a count of the commands run would
be truthful and still misleading. On an Opaque `mutate` it is accepted and the same reasoning holds of
it, so what says so is the review's `unbounded` flag, which such a Step draws whether it declares a
Bound or not.
...
**Secret sink**:
The destination an invocation supplies for output an Operation declares secret: a **directory** `hyper`
...
skips being distinct because only the first concluded about anything, and the two attempts because only
