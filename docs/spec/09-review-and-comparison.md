...
four remedies, none of them an edit and all of them keeping `77`'s promise that
...
surface, under any flag. Nor where the Operation declares secret output: the projection carries the
marker in each declared position as a Record's fields do, and the response object is withheld whole,
the marker standing in its place (ADR-0017). What fills that gap is precision rather than volume: a projection that failed
