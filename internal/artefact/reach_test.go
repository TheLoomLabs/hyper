...
// TestResolveHost is ADR-0029's three steps walked at invocation, and the
...
func TestResolveHost(t *testing.T) {
	...
	for _, c := range []struct {
	...
	}{
		{
			...
			want:      artefact.HostReach{Reach: artefact.ReachGranted, Host: "status.hyper.dev"},
		},
		{
			...
			want:      artefact.HostReach{Reach: artefact.ReachGranted, Host: "api.cloudflare.com"},
		},
		{
			name:      "a literal host the grant does not admit",
			operation: artefact.OperationInfo{HostTemplate: "api.cloudflare.com"},
			target:    granted("status.hyper.dev"),
			want:      artefact.HostReach{Reach: artefact.ReachNotGranted, Host: "api.cloudflare.com"},
		},
		{
			...
			want:      artefact.HostReach{Reach: artefact.ReachGranted, Host: "cert.hyper.dev"},
		},
		{
			...
			want:      artefact.HostReach{Reach: artefact.ReachNotGranted, Host: "elsewhere.example.com"},
		},
		{
			name:      "a grant of nothing at all names no host to decline over",
			operation: artefact.OperationInfo{HostTemplate: "{from-target}"},
			target:    granted(),
			want:      artefact.HostReach{Reach: artefact.ReachNotGranted},
		},
		{
			name:      "two granted candidates under no host-input: decide nothing",
			operation: artefact.OperationInfo{HostTemplate: "{from-target}"},
			target:    granted("status.hyper.dev", "cert.hyper.dev"),
			want:      artefact.HostReach{Reach: artefact.ReachUndecidable, Granted: 2},
		},
		{
			...
			want:      artefact.HostReach{Reach: artefact.ReachGranted, Host: "s3.us-east-1.amazonaws.com"},
		},
		{
		...
		},
		{
			...
			want:      artefact.HostReach{Reach: artefact.ReachIllegible},
		},
	} {
		t.Run(c.name, func(t *testing.T) {
			got := artefact.ResolveHost(c.provider, c.operation, c.target, c.supplied)
			if got != c.want {
				t.Errorf("ResolveHost = %+v, want %+v", got, c.want)
			}
		})
	}
}
