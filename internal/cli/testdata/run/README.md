...
| `a-procedure-matching-nothing`, `a-definition-rather-than-a-procedure`, `a-path-rather-than-a-procedures-name`, `two-positionals`, `a-target-flag` | the five usage errors, all `2`, all with stdout completely silent. The third is the one no case drove until issue #207: this command's positional is a Procedure's **name** and takes no path form, so `procedures/watch-status.yaml` is a name that resolves to nothing — and it says which grammar it takes rather than which namespace it missed (ADR-0090) |
...
| `a-manifest-taking-the-built-ins-name` | §11's first rule about what an Extension may never be, reaching the same pre-flight (issue #185): a `providers/shell.yaml` taking the built-in `shell` Provider's name, `provider-name-collision` at the Run-start `check`, `77`, and the caret on the `provider:` scalar. Its remedy **is** an artefact edit, so it renders an `EDIT ONE OF` where `a-stale-projection` renders a command, and the Procedure it declines resolves its Step through the built-in — the Run refuses over the file that took the name rather than over anything the Definition did |
| `a-manifest-declaring-the-reserved-capability` | §11's other rule about what an Extension may never be (issue #186): a `providers/deploy-helper.yaml` declaring `shell` and carrying a `shell:` request block — a Manifest that satisfies the declared-equals-derived check exactly — refuses `capability-reserved` at `77`, once at each spelling. The Procedure it declines resolves its own Step through the **built-in**, which is the point: the Run refuses over a file no Step of it reaches, because what `check` re-runs at Run start is the repository and not the path through it. Without this the file is one `hyper run` away from exec'ing argv on the machine, which is the guarantee §11 states in words |
| `an-installed-manifest-whose-bytes-moved` | §11's third Extension code at the same pre-flight (issue #189), and the one that is not about what an Extension may never be but about what an installed one must still be: a `providers/dns.yaml` fetched from a registry and edited by one byte afterwards — `deadline: 30s` become `deadline: 45s` — refuses `origin-digest-mismatch` at `77`, the caret on the `digest:` scalar and both digests named whole. It is here for the reason the two above it are, and one of its own: the digest is what makes a reviewed Extension the *reviewed* Extension, and a rule that held on a laptop and not on the runner would exempt the one occasion nobody is reading the diff |
| `a-manifest-written-in-a-shape-this-hyper-does-not-know` | §11's fourth Extension code at the same pre-flight (issue #190), and the one that presupposes the other three: a `providers/inventory.yaml` declaring `schema-version: 2` refuses `manifest-schema-unsupported` at `77`, the caret on that scalar. Its two deliberate faults beneath it — a sixth top-level key and a `capabilities:` list its own Operation disagrees with — are **not** reported, which is the whole of the rule: a Manifest read on a partial understanding of its own shape has its declared-equals-derived Capability check run against keys the reader could not see. It is also the second Refusal here citing an **artefact** whose remedy is not an edit — `a-credential-the-environment-does-not-hold` is the first — a named remedy, *a hyper that reads this schema version*, where `a-manifest-taking-the-built-ins-name` renders an `EDIT ONE OF` |
...
| `a-working-tree-edited-since-check-passed` | the same gate driven the way an operator meets it — one `uncommitted/` line narrows `local`'s `kinds:`, and the Run refuses with the codes the edit earns. Its `stderr.golden` carries the uncommitted warning too, and this is where that line is at its plainest: it is written before the Run's own gate declines it, the tree having been read before anything was checked (ADR-0119) |
...
| `a-credential-set-to-the-empty-string` | the same slot and the same `env:` line one state over: `PAID_TOKEN` exported to nothing refuses `credential-empty` at `77` rather than composing into `Bearer ` and earning a `401` at `1`. It is the sibling of the case above and drives what the split bought — its own message, and its own `=` remedy — *check what left it empty* rather than *set it in the environment*, which is the one advice a variable that is already exported cannot use (§12, ADR-0145, issue #264) |
...
| `one-absent-slot-and-one-empty-one` | the same pass reporting **both** its codes in one array: `PAID_TOKEN` exported to nothing and `VAULT_PASSWORD` never exported, ordered by file like every array (§7), so the first member — `credential-empty` — is what the terminal line and `outcome.json` name. A pass that reported one kind and stopped, or that sorted its own codes ahead of the file order, would read the same on a single-fault repository and differently here (issue #264) |
...
| `a-rehearsal-withholds-a-nested-step`, `-json` | the withheld Step reached through an invocation: the page names it `publish-inner.publish`, under the path the table renders it at, so the sentence and the row point at one Step rather than at two spellings of one (§8). The `-json` half is where `withheld: true` and `path` are on one row, which is the pair no other rehearsal golden holds — the page composes the two into a name and the row carries them apart, as the Step file does (ADR-0091, issue #206) |
