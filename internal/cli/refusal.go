...
//
// All four of §8's classes have members and all four are here. A **command** —
// `hyper project`, `hyper store init`. A **different binary**, which is the one
...
// back to — the code standing in its place was `secret-sink-unwritten`, whose
// remedy was a binary, and naming the flag would have ended the round trip on
// another `77` (ADR-0146, ADR-0145). Now that the file is written, the flag is
// the remedy and the round trip ends on a Run that completes (ADR-0148).
//
