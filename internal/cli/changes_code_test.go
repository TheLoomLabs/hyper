...
func TestCodeMoved_EveryClassEmitsOneRowPerSubjectAndFact(t *testing.T) {
	...
	want := strings.Join([]string{
		...
		"                                                         scheduled runs happen on the default branch only                                             scheduled runs happen on the default branch only",
		"                                                         :00 is the executor's busiest minute — delivery there is likeliest to be delayed or dropped  more than one run an hour is more than the executor delivers — most occurrences will never fire",
